...
public class TxHandler {
    ...
    /**
     * Creates a public ledger whose current bchain_mini.UTXOPool (collection hashOf unspent transaction outputs) is
     * {@code utxoPool}. This should make a copy hashOf utxoPool by using the bchain_mini.UTXOPool(bchain_mini.UTXOPool uPool)
     * constructor.
     */
     ...
     * (2) the signatures on each input hashOf {@code tx} are valid,
     * (3) no bchain_mini.UTXO is claimed multiple times by {@code tx},
     * (4) all hashOf {@code tx}s output values are non-negative, and
     * (5) the sum hashOf {@code tx}s input values is greater than or equal to the sum hashOf its output
    ...
    /**
     * Handles each epoch by receiving an unordered array hashOf proposed transactions, checking each
     * transaction for correctness, returning a mutually valid array hashOf accepted transactions, and
